-- Sky.lua - Sky Squid Game модуль для Creon X v2.1
...
-- Anti Fall функция для Sky Squid
    ...
    if enabled then
        voidKillConnection = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                -- Проверка, находимся ли мы в бою с другим игроком
                for _, player in pairs(Players:GetPlayers()) do
                    if player ~= LocalPlayer and player.Character then
                        local targetRoot = player.Character:FindFirstChild("HumanoidRootPart")
                        if targetRoot then
                            ...
                            if distance < 15 then -- Дистанция боя
                                -- Телепортация в пустоту и создание платформы
                                local voidPosition = Vector3.new(0, 10000, 0) -- Далеко над землей
                                player.Character.HumanoidRootPart.CFrame = CFrame.new(voidPosition)
                                
                                -- Создание платформы под врагом
                                ...
                                -- Очистка платформы через некоторое время
                                delay(10, function()
...
-- Safe Platform функция
function SkySquidModule.ToggleSafePlatform(enabled)
    if enabled then
        if SkySquidModule.SafePlatform and SkySquidModule.SafePlatform.Parent then
        ...
        SkySquidModule.SafePlatform = Instance.new("Part")
        SkySquidModule.SafePlatform.Name = "SkySquid_SafePlatform"
        SkySquidModule.SafePlatform.Size = Vector3.new(500, 5, 500)
        SkySquidModule.SafePlatform.Position = Vector3.new(rootPart.Position.X, 100, rootPart.Position.Z)
        SkySquidModule.SafePlatform.Anchored = true
        SkySquidModule.SafePlatform.CanCollide = true
        SkySquidModule.SafePlatform.Transparency = 0.7
        SkySquidModule.SafePlatform.Material = Enum.Material.Neon
        SkySquidModule.SafePlatform.BrickColor = BrickColor.new("Bright blue")
        SkySquidModule.SafePlatform.Parent = workspace
        
        print("Safe Platform создана для Sky Squid на высоте Y = 100")
    else
...
-- Auto QTE функция для Sky Squid (опционально)
function SkySquidModule.ToggleAutoQTE(enabled)
    SkySquidModule.AutoQTEEnabled = enabled
    
    if SkySquidModule.AutoQTEConnection then
        SkySquidModule.AutoQTEConnection:Disconnect()
        SkySquidModule.AutoQTEConnection = nil
    end
    
    if enabled then
        SkySquidModule.AutoQTEConnection = RunService.Heartbeat:Connect(function()
...
print("[Sky.lua] Sky Squid Game модуль загружен!")
print("[Sky.lua] Доступные функции:")
print("[Sky.lua] 1. Anti Fall - Sky Squid")
print("[Sky.lua] 2. Void Kill")
print("[Sky.lua] 3. Safe Platform - Sky Squid")
print("[Sky.lua] 4. Auto QTE - Sky Squid (опционально)")
